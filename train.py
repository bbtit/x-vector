...
スピーカーIDシステムのトレーニング用レシピ。このテンプレートは、言語ID、感情認識、コマンド分類など、
あらゆる信号分類タスクの基本例として使用できる。提案するタスクは、Mini Librispeechを用いて28人の話者を分類する。

このタスクは非常に簡単である。実際のシナリオでは、voxcelebのような話者数の多いデータセットを使用する必要があります（recipes/VoxCelebを参照）。

Speechbrainには、シグナル分類のためのビルトインモデルがすでにいくつかあります
（speechbrain.loves.models.ECAPA_TDNN.pyのECAPAや、speechbrain/lobes/models/Xvector.pyのxvectorを参照してください）。

このレシピを実行するには、次のようにします：
> python train.py train.yaml

コードを読むには、まず一番下までスクロールして "main "コードを見てください。
Brainクラスの定義がトレーニング中に各バッチで起こることの詳細を提供する一方で、これは何が起こっているかのハイレベルな概要を提供します。

最初にこのスクリプトを実行すると、自動的にMini Librispeechデータセットがダウンロードされ、計算の準備が行われます。
ノイズと残響は、OpenRIRから各サンプルに自動的に追加されます。
...
class SpkIdBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        N個のクラスに対する入力を出力確率に変換するすべての計算を実行する。
...
def dataio_prep(hparams):
    ...
    are available.
    この関数は、brainクラスで使用するデータセットを準備する。
    また、ユーザー定義関数によるデータ処理パイプラインも定義する。
    この関数の前に `prepare_mini_librispeech` が呼び出され、
    `train.json`、`valid.json`、`valid.json` マニフェストファイルが利用可能になっていることを期待します。
...
if __name__ == "__main__":
    ...
    # マルチGPU DDPトレーニングにのみ有用
    sb.utils.distributed.ddp_init_group(run_opts)

    # Load hyperparameters file with command-line overrides.
    # ハイパーパラメータファイルをコマンドラインで上書きする箇所も含めて読み込む
    ...
    # fit()`メソッドはトレーニングループを繰り返し、モデルのパラメータを更新するために必要なメソッドを呼び出す。
    # 状態が変化する全てのオブジェクトはチェックポインタによって管理されるので、
    # トレーニングはどの時点でも停止することができ、次の呼び出しで再開される。
    spk_id_brain.fit(
